...
Iref = imread('C:\Users\hfsul\Documents\GitHub\BA-Path-planning\Image Processing\Calibration etc\New_Reference_Color_Color_Color.png');      % empty table
...
interiorMask = false(size(BW_limited));
interiorMask(yi:ye, xi:xe) = true;

insideBW = BW_limited & interiorMask;
%figure; imshow(insideBW); title('bef dil');
if safety_dilate_px > 0
    se = strel('disk', safety_dilate_px);
    insideBW = imdilate(insideBW, se) & interiorMask;
end
figure; imshow(insideBW); title('aft dil');
...
% Inflate for safety
inflate = 10;  % mm
insideBW = imdilate(insideBW, strel('disk', inflate));
figure; imshow(insideBW);title('Inflate for safety');
...
insideBW = 1-insideBW;
figure; imshow(insideBW);title('Rectangle insetion');
